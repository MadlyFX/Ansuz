...
emit_deferred_draws :: proc(mgr: ^Manager) {
	...
	for &dd in mgr.deferred_draws {
		...
		switch dd.kind {
		case .Slider:
			emit_slider_draw(mgr, r, dd.slider, s)
...
emit_slider_draw :: proc(mgr: ^Manager, rect: Rect, data: Deferred_Slider_Data, scale: f32 = 1.0) {
	...
	push_filled_rect(&mgr.draw_list, track_rect, THEME_SLIDER_TRACK, radius)
	...
	if data.t > 0 {
		fill_color := THEME_SLIDER_FILL_HOVER if .Hovered in data.interaction else THEME_SLIDER_FILL
	...
	// Thumb
	thumb_color := THEME_SLIDER_THUMB_ACTIVE if .Pressed in data.interaction else THEME_SLIDER_THUMB
...
emit_dropdown_list :: proc(mgr: ^Manager, anchor: Rect, data: Popup_Dropdown_Data) {
	...
	for opt, i in data.options {
		...
		// Item text
		text_dims := measure_text_builtin(opt, DEFAULT_FONT_SCALE)
		tx := item_rect.x + 10
		ty := item_rect.y + (item_h - text_dims.y) / 2
		push_text(&mgr.draw_list, {tx, ty}, opt, THEME_TEXT, 0, DEFAULT_FONT_SCALE)
...
emit_text_cursor_draw :: proc(mgr: ^Manager, cr: Rect, data: Deferred_Text_Cursor_Data, scale: f32) {
	...
	if data.multiline {
		...
		cursor_x = cr.x + f32(col) * char_w
		cursor_y = cr.y + f32(line) * char_h
	} else {
		cursor_x = cr.x + f32(data.cursor_pos) * char_w
		cursor_y = cr.y + (cr.h - char_h) / 2
	}

	thickness := max(1, 2 * (scale / DEFAULT_FONT_SCALE))
	push_filled_rect(&mgr.draw_list, Rect{cursor_x, cursor_y, thickness, char_h}, THEME_TEXTINPUT_CURSOR)
}
